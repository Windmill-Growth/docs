...
Lead, lead signal, and account report endpoints return leads with an ICP match of at least 50% by default. Pass `minIcpMatchPercent` to change that minimum. For example, `minIcpMatchPercent=0` also returns qualified engagers with a lower ICP match. Use `maxIcpMatchPercent` to set an upper bound.
...
- monitored profile
- LinkedIn post
- engagement type, including post author
- owner
- company
- text query
- date range
...
Use `GET /customer/analytics/meetings` for Meetings booked counts and `GET /customer/meetings` for the attributed meetings list. The meetings list accepts `range`, `leadId`, and `targetType`. Meeting attendee details require `leads:contact_read`.
...
Any active workspace member can connect MCP. The approval screen shows the permissions available to your role in each workspace, and you choose what to grant. ICP and Watchlist changes require an owner or admin. Every tool call also checks your current role, so a role change takes effect on the next request.

One connection can include up to 50 workspaces that you select during approval. `traxy_list_workspaces` returns the workspaces available to the connection. Other tools accept an optional `workspaceId`. When you omit it, traxy uses the default workspace from the approval.

`tools/list` returns only the tools allowed by the approved scopes and current workspace access.

## MCP tools

The current catalog has 48 scope-gated tools.

### Connection and workspace

- `traxy_list_workspaces`
...
- `traxy_update_active_icp`
- `traxy_get_active_signal_brief`
- `traxy_update_active_signal_brief`
...
ICP and Signal Brief updates require `icp:read` and `icp:manage`. Arrays you send replace the current values, and fields you omit stay unchanged.
...
- `403` when a browser origin is not allowed to call MCP
- `404` when the requested resource is not available to the workspace
- `409` for a malformed or unsupported LinkedIn post reference
- `429` for rate limits
- `503` when Meetings data is temporarily unavailable. Retry after the `Retry-After` delay.
